...
@section('content')
<section class="max-w-5xl mx-auto p-6" x-data="{ showModal: false }">
    ...
    <div class="flex items-center justify-between mb-8 border-b border-gray-100 dark:border-gray-700 pb-6">
        <div>
            <h2 class="text-3xl font-extrabold text-gray-900 dark:text-white uppercase tracking-tight">
                Crestec Philippines - CSR Updates
            </h2>
        ...
        <article class="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
            ...
            @if($singlePost->image && is_array($singlePost->image))
                <div x-data="{ activeImage: 0, images: {{ json_encode($singlePost->image) }} }" class="relative group mb-8">
                    ...
                    <div class="relative overflow-hidden rounded-2xl shadow-lg bg-gray-100 dark:bg-gray-900 h-[500px]">
    ...
    @else
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            @forelse($posts as $post)
                ...
                <div class="px-6 pb-6 pt-4 mt-auto grid grid-cols-3 items-center border-t border-gray-50 dark:border-gray-700 min-h-[60px]">
                    ...
                    <div class="flex justify-end">
                        @auth
                            @if(auth()->id() === $post->user_id)
                                <form method="POST" action="{{ route('posts.destroy', $post) }}" onsubmit="return confirm('Delete this post?');" class="flex items-center">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="text-gray-400 hover:text-red-600 transition-colors p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center justify-center">
        ...
        <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full z-50">
            <form method="POST" action="{{ route('posts.store') }}" enctype="multipart/form-data">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">When did this happen?</label>
                        ...
                        <p class="text-[10px] text-gray-500 mt-1 italic">Defaults to today if left blank.</p>
                    </div>
